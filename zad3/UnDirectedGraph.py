...
class UnDirectedGraph: 
    ...
    def dfs(self, graph, node, visited, parent, toPrint):
        visited[node] = True
        for neighbor in range(len(self.adjMatrix[node])):
            if self.adjMatrix[node][neighbor] == 1 and not visited[neighbor]:
                graph.edge(str(node), str(neighbor),  color='red')
                toPrint.append(neighbor)
                self.dfs(graph, neighbor, visited, node, toPrint)
                                        
    def dfsCheck(self):
        if self.isGraphSimple() == False or self.isGraphConnected() == False:
            return None, None
        else:
            dfsGraph = Graph()
            for i in range(self.numNodes):
            ...
            for node in range(self.numNodes):
                if not visited[node]:
                    toPrint.append(node)
                    self.dfs(dfsGraph, node, visited, None, toPrint)
            
            return dfsGraph, toPrint
